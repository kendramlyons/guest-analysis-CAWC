...
countries %>%
  ...
  ggplot(aes(y = reorder(country, percent),  
             x = percent)) +
  geom_col() +
  labs(title = "Casa Alitas Arrivals by Origin Country", 
       y = element_blank(), 
...
individuals %>%
  ...
  labs(title = "Casa Alitas Daily Arrivals", 
...
languages %>%
  ...
  ggplot(aes(y = reorder(language, percent), 
             x = percent)) +
  geom_col() +
  labs(title = "Casa Alitas Arrivals by Language", 
       y = element_blank(), 
       x = "Percent of Arrivals")+
...
destinations %>%
  filter(percent > .5) %>%
  ggplot(aes(y = reorder(state_name, percent), 
             x = percent)) +
  geom_col() +
  #theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5)) +
